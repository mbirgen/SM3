...
`r index_entry('C', 'computation!foundations')`
...
`r index_entry('C', 'Chambers, John')` refers to the "Prime Directive" of
`r index_entry('C', 'software!prime directive')`
softwareTEX COMMAND NOT FOUND cite chambers-2008 : "to program in such a way that
...
is the spreadsheet.  Examples are Excel `r index_entry('C', 'Excel')` and Google Spreadsheets.
`r index_entry('Google spreadsheets')`
`r index_entry('spreadsheets')` 
...
The syntax `r index_entry('C', 'syntax!in R')`  `r index_entry('R!syntax')`
